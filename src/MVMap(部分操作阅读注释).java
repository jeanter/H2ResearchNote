    ...
    public V operate(K key, V value, DecisionMaker<? super V> decisionMaker) {
        ...
        while(true) {
            ...
            try {
                ...
                LogUtils.log("start operate key "+ key +" mapID ("+ getId() + ") ************  current p info :"+ PageUtil.getPageInfo( p ) +"  attempt = "+attempt);
                try {
            		key.toString().charAt(100);
            	}catch (Throwable t) {
            		t.printStackTrace();
            	}
                switch (decision) {
                    ...
                    case REMOVE: {
                        ...
                        if (p.getTotalCount() == 1 && pos != null) {
                        	LogUtils.log("remove key "+ key +"  p.getTotalCount() == 1 " +" current p info :"+ PageUtil.getPageInfo(p));
                            int keyCount;
                            do {
                            	// p现在为p的父节点
                                p = pos.page;
                            ...
                            if (keyCount <= 1) {
                            	LogUtils.log("remove key keyCount <= 1   "+ key  +",keyCount : "+keyCount + "; current p info :"+ PageUtil.getPageInfo(p));
                                if (keyCount == 1) {
                                    assert index <= 1;
                                    //p又指向了新的叶子节点
                                    p = p.getChildPage(1 - index);
                                    LogUtils.log(" remove key no remove "+ key + " ,current p info :"+ PageUtil.getPageInfo( p ));
                                } else {
                                    ...
                                    LogUtils.log(" remove key else no remove "+ key + " ,current p info :"+ PageUtil.getPageInfo( p ));
                                }
                                //p指向了新的叶子节点 直接返回 ，不用再删除了任何节点了  需要被删除的子节点已经没有对其的任何引用了
                                break;
                            }
                        }
                        // 如果分支1成立，p此时是leaf的父节点. 调用NonLeaf.remove(index)  先remove key,再remove child
                        //如果分支1不成立 p 此时是叶子节点,直接删除key和value
                        LogUtils.log("start***** remove key "+ key + " ,current p info :"+ PageUtil.getPageInfo( p ));
                        p = p.copy();
                        p.remove(index);
                        LogUtils.log("end***** remove key "+ key + " ,current p info :"+ PageUtil.getPageInfo( p ));
                        break;
                    }
                    case PUT: {
                ...
                LogUtils.log("end operate key "+ key + " ,replacePage************  current p info :"+ PageUtil.getPageInfo( p ));
                if (!locked) {
                	//通过cas的方式修改map上的rootReference ,如果map上的rootReference还是一开始获取的那个对象 说明root没有被其他对象修改
                	//就将map上的rootReference修改成新的RootReference
                ...
                if(locked) {
                ////通过cas的方式修改map上的rootReference为最新的rootReference
                    unlockRoot(rootPage);
                }
            }
        }
